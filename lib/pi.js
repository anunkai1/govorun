...
// Pi's agent_end includes every message and tool result from a run. A document
// workflow can make that aggregate exceed the bounded RPC line size, while the
// final turn_end is small and is emitted only after the final assistant message
// has been persisted. Prefer it as the completion signal.
export function completedResponse(record, streamedText = "") {
  if (record?.type === "turn_end" && record.message?.role === "assistant" && record.message.stopReason === "stop") {
    return streamedText.trim() || assistantText(record.message).trim() || "Done.";
  }
  if (record?.type !== "agent_end") return null;
  const messages = Array.isArray(record.messages) ? record.messages : [];
  const lastAssistant = messages.findLast((message) => message?.role === "assistant");
  return streamedText.trim() || assistantText(lastAssistant).trim() || "Done.";
}
...
export class GroupAgent {
  ...
  spawn(modelEntry) {
    ...
    const env = {
      HOME: "/home/govorun",
      PATH: `/opt/govorun/app/bin:${this.group.workspace}/bin:${this.group.workspace}/.tools/venv/bin:/home/govorun/.local/bin:/home/govorun/.npm-global/bin:/usr/local/bin:/usr/bin:/bin`,
      NODE_ENV: "production",
      GOVORUN_WORKSPACE: this.group.workspace,
      GOVORUN_BIN_DIR: join(this.group.workspace, "bin"),
      GOVORUN_OUTBOX: join(this.group.workspace, ".govorun-outbox"),
      GOVORUN_SELF_ROOT: "/home/govorun/self/current",
      GOVORUN_RELOAD_PID: String(process.pid),
      PI_VISION_PROXY_MODE: "fallback",
      PI_VISION_PROXY_TOOL: "on",
      PI_VISION_PROXY_MODEL: "openai-codex/gpt-5.6-luna",
      PI_OFFLINE: "1",
      ...(process.env.VENICE_API_KEY ? { VENICE_API_KEY: process.env.VENICE_API_KEY } : {}),
      ...(process.env.BU_CDP_URL ? { BU_CDP_URL: process.env.BU_CDP_URL } : {}),
    };
  ...
  promptWith(model, message) {
    return new Promise((resolve, reject) => {
      ...
      parseRpc(proc, (record) => {
        ...
        const response = completedResponse(record, text);
        if (response !== null) finish(resolve, response);
